...
<div id="content">
    ...
    if ($cart_list) {
        foreach ($cart_list as $data) {
            ...
            $discount = getDatabaseTicketValue(getDatabaseCartTicketSelectTicketID(getDatabaseCartTicketSelectByCartID($cart_product_list_id)));
            ?>
            <label class="order">
                ...
                <span class="value">R$ <?php echo doCartTotalPriceProduct($cart_product_list_id) ?></span>
            </label><br>
            <hr>
            <?php
        }
    ...
    <label>Desconto: <span class="value">
            ...
            <?php echo getDatabaseTicketCode(getDatabaseCartTicketSelectTicketID(getDatabaseCartTicketSelectCartID($cart_product_list_id))) ?>
            <small>Desconto de
                [<?php echo ($discount !== false) ? $discount : 'Nenhum desconto selecionado.' ?>]</small>

        </span>
    </label><br>
    <label>Total: <span class="value">R$ <?php echo (doCartTotalPrice($order_id) - doCartTotalPriceDiscount($order_id)) ?></span>
    </label><br>
    <!-- <hr>
